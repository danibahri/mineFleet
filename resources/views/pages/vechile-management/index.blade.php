            ...
            <table class="w-full min-w-[860px] table-auto text-sm">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($this->vehicles as $vehicle)
                        @php
                            $statusLabel = $this->statusOptions()[$vehicle->status] ?? ucfirst($vehicle->status);
                            $statusClass = match ($vehicle->status) {
                                ...
                                'service'   => 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-300',
                        ...
                        <tr class="text-slate-700 dark:text-slate-200">
                            ...
                            <td class="py-3 text-right">
                                <div class="flex flex-wrap items-center justify-end gap-2">
                                    <flux:button size="sm" variant="ghost" wire:click="openEditForm({{ $vehicle->id }})">
                                        Edit
                                    </flux:button>
            ...
            <div class="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900 max-h-[90vh] overflow-y-auto">
                <div class="mb-5 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400">
                            {{ $vehicleId ? 'Perbarui data kendaraan.' : 'Isi detail kendaraan baru.' }}
                        </p>
                    </div>
                ...
                <div class="space-y-4">
                    <div class="grid gap-3 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model.defer="plateNumber"
                                class="mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:bg-slate-900" />
                            @error('plateNumber') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-3">
                        <div>
                            ...
                            <input type="number" wire:model.defer="year"
                                class="mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:bg-slate-900" />
                            @error('year') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            ...
                            @error('ownershipType') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                        <div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">Jenis Kendaraan</label>
                            ...
                            @error('formVehicleTypeId') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" wire:model.defer="fuelType"
                                class="mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:bg-slate-900" />
                            @error('fuelType') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">Fuel Consumption</label>
                            <input type="number" step="0.01" wire:model.defer="fuelConsumption"
                                class="mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:bg-slate-900" />
                            @error('fuelConsumption') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="number" wire:model.defer="odometer"
                                class="mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:focus:bg-slate-900" />
                            @error('odometer') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </div>
                        <div>
